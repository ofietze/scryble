...
static uint16_t get_num_rows(MenuLayer *ml, uint16_t sec, void *ctx) { return 8; }
...
static void select_click(MenuLayer *ml, MenuIndex *idx, void *ctx) {
  g_state.filter.type = (uint8_t)idx->row;
  filter_alpha_window_push();
}
...
void filter_type_window_push(void) {
  if (!s_window) {
